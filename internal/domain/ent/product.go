...
type Product struct {
	...
	ClayfulOptions any `json:"clayful_options,omitempty"`
	...
	for i := range columns {
		...
		case product.FieldClayfulID:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				pr.ClayfulID = value.String
			}
		case product.FieldClayfulOptions:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field clayful_options", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &pr.ClayfulOptions); err != nil {
					return fmt.Errorf("unmarshal field clayful_options: %w", err)
				}
			}
		default:
...
func (pr *Product) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("clayful_options=")
	builder.WriteString(fmt.Sprintf("%v", pr.ClayfulOptions))
	builder.WriteByte(')')
